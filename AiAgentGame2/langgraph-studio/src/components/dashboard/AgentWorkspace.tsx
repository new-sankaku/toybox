...
function AgentCharacterCard({
  ...
  // Start exit animation when completed (with delay based on initial position)
  useEffect(() => {
    if (isCompleted && !isExiting && !exitTimerRef.current) {
      // Delay based on initial position - left agents exit first
...
export default function AgentWorkspace(): JSX.Element {
  ...
  // Get agents for current project (fixed order by phase and type)
  const displayAgents = useMemo(() => {
    ...
    // Sort by phase, then by type (alphabetically) - position never changes
