...
Code Leaderは以下のループでサブAgentを管理しタスクを完遂します：

### ループ図

```
┌─────────────────────────────────────────────────────────────┐
│                   CODE LEADER TASK LOOP                      │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
                    ┌─────────────────┐
...
| ステップ | 処理内容 | 成果物 |
|---------|---------|-------|
| 1. タスク受領 | イテレーションのコードタスク取得 | `code_tasks[]` |
| 2. サブAgent生成 | 必要なサブAgentを動的作成 | `sub_agents[]` |
| 3. タスク選択 | 依存関係順に次タスク取得 | `current_task` |
| 4. Agent割当 | 最適なサブAgentを選択 | `assignment` |
| 5. 実装実行 | サブAgentがコード生成 | `code_output` |
| 6. レビュー | 品質と整合性を確認 | `review_result` |
| 7. 成果物登録 | 完了タスクとコードを記録 | `completed_tasks[]` |
| 8. 次へ | 残タスクがあれば継続 | `pending_tasks[]` |
| 9. 完了報告 | イテレーション成果物を出力 | `CodeLeaderOutput` |

### 品質チェック基準

各タスクの成果物は以下を満たすまでループ：

```typescript
interface CodeReviewValidation {
  // コード品質
  compiles_without_error: boolean;     // コンパイルエラーなし
  passes_unit_tests: boolean;          // ユニットテスト合格
  follows_coding_style: boolean;       // コーディング規約準拠

  // 設計整合性
  matches_design_spec: boolean;        // 設計書と一致
  proper_interfaces: boolean;          // インターフェース適切
  no_magic_numbers: boolean;           // マジックナンバーなし

  // 機能性
  meets_acceptance_criteria: boolean;  // 受入条件達成
  handles_edge_cases: boolean;         // エッジケース対応

  // 保守性
  is_readable: boolean;                // 可読性良好
  is_documented: boolean;              // 必要なコメントあり
...
```typescript
function selectSubAgent(task: CodeTask): SubAgent {
  switch (task.component_type) {
    case "core":
      return getOrCreateAgent("CoreAgent");
    case "system":
      return getOrCreateAgent("SystemAgent");
    case "scene":
      return getOrCreateAgent("SceneAgent");
    case "ui":
      return getOrCreateAgent("UIAgent");
    case "test":
      return getOrCreateAgent("TestAgent");
    default:
      return getOrCreateAgent("GeneralCodeAgent");
  }
}
```
