...
Character Agentは以下のループで各キャラクターを設計します：
...
| ステップ | 処理内容 | 成果物 |
|---------|---------|-------|
| 1. リスト作成 | シナリオから必要キャラを抽出 | `pending_characters[]` |
| 2. キャラ選択 | 重要度順に次を取得 | `current_character` |
| 3. 基本設定 | 名前、年齢、役割を定義 | `basic_info` |
| 4. 性格・内面 | 心理的深みを追加 | `personality` |
| 5. 外見 | ビジュアル指針を設計 | `visual_design` |
| 6. 関係性 | 他キャラとの繋がり定義 | `relationships` |
| 7. ゲームプレイ | 能力とスキルを設計 | `gameplay_stats` |
| 8. 次へ | 完了リストに追加 | `completed_characters[]` |
| 9. 関係性検証 | 全体の整合性確認 | `CharacterOutput` |
...
interface CharacterValidation {
  // 個性チェック
  is_memorable: boolean;           // 記憶に残る特徴があるか
  is_distinct: boolean;            // 他キャラと区別できるか
  has_clear_motivation: boolean;   // 動機が明確か

  // 深みチェック
  has_flaws: boolean;              // 欠点があるか（完璧すぎないか）
  has_growth_potential: boolean;   // 成長の余地があるか
  backstory_supports_personality: boolean; // 背景と性格が一致するか

  // 機能チェック
  serves_story_purpose: boolean;   // ストーリー上の役割があるか
  has_gameplay_function: boolean;  // ゲームプレイ上の機能があるか

  // ビジュアル
  silhouette_unique: boolean;      // シルエットで区別できるか
