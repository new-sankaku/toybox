...
class TestDataStore:
    ...
    def _init_sample_data(self):
        ...
        # Create agents matching workflow structure (9 phases: 0-8)
        # Phase 0: 企画 - コンセプト
        # Phase 1: タスク分割1
        # Phase 2: 設計 - コンセプト詳細/シナリオ/世界観/ゲームデザイン/技術仕様 (L/W統合)
        # Phase 3: タスク分割2
        # Phase 4: アセット - キャラ/背景/UI/エフェクト/BGM/ボイス/効果音
        # Phase 5: タスク分割3
        # Phase 6: 実装 - コード/イベント/UI統合/アセット統合 (L/W統合)
        # Phase 7: タスク分割4
        # Phase 8: テスト - 単体テスト/統合テスト (L/W統合)
        # L/W = Leader/Worker継続的ループチェック
        agents_data = [
            # Phase 0: 企画
        ...
        # Start with empty assets - will be generated by agents
        self.assets[proj_id] = []
        print(f"[TestDataStore] Initialized with empty assets (will be generated by agents)")
    ...
    def _can_start_agent(self, agent_type: str, agents: List[Dict]) -> bool:
        ...
        for dep_type in dependencies:
            ...
            # Cannot start if dependency is not completed or is waiting for approval
    ...
    def _simulate_project(self, project_id: str):
        ...
        if running_agents:
            # Simulate all running agents (parallel execution)
            for agent in running_agents:
                self._simulate_agent(agent)
        else:
            # Start next agents based on workflow dependencies
            ready_agents = self._get_next_agents_to_start(agents)
            if ready_agents:
                # Start all ready agents (enables parallel Phase 2)
    ...
    def _simulate_agent(self, agent: Dict):
        # Skip if already waiting for approval
        ...
        # If checkpoint was created, agent is now waiting_approval - don't complete
        if agent["status"] == "waiting_approval":
    ...
    def _create_asset(self, agent: Dict, asset_type: str, name: str, size: str):
        ...
        # Try to find a real file from testdata folder
        testdata_path = get_testdata_path()
        real_file = self._find_real_file(testdata_path, asset_type, project_id)

        if real_file:
            # Use real file from testdata
            asset = {
            ...
            display_name = agent["metadata"].get("displayName", agent["type"])
            self._add_system_log(project_id, "info", display_name, f"アセット生成: {real_file['name']}")
        else:
            # Fallback to generated asset info
            if asset_type == "image":
                url = f"/assets/{name}"
        ...
        # Emit WebSocket event for new asset
        self._emit_event("asset:created", {
            "projectId": project_id,
            "asset": asset
        }, project_id)

    def _find_real_file(self, testdata_path: str, asset_type: str, project_id: str) -> Optional[Dict]:
        # Map asset type to testdata subdirectory
        subdir_map = {
            "image": "image",
            "audio": "mp3",
            "video": "movie"
        ...
        # Get already used files in this project
        used_files = set()
        for asset in self.assets.get(project_id, []):
            if asset.get("url"):
                used_files.add(asset["url"])

        # Find unused file
        available = [f for f in all_files if f"/testdata/{f['relative']}" not in used_files]
        if not available:
            # All files used, pick random
            available = all_files
...
- 空中: 浮遊感と開放感"""
    ...
    def initialize_project(self, project_id: str) -> Optional[Dict]:
        with self._lock:
            ...
            # Recreate agents matching workflow structure (9 phases: 0-8)
            # L/W統合済み: Phase 2/6/8 のL/W対は1ノードに統合
            # タスク分割は独立Phaseとして配置 (P1, P3, P5, P7)
            agents_data = [
                # Phase 0: 企画
                {"type": "concept", "name": "コンセプト", "phase": 0},
                # Phase 1: タスク分割1
            ...
            # Clear assets - will be generated by agents
            self.assets[project_id] = []
    ...
    def resolve_checkpoint(self, checkpoint_id: str, resolution: str, feedback: Optional[str] = None) -> Optional[Dict]:
        with self._lock:
            ...
            # Update agent status back to running if no other pending checkpoints
            agent_id = checkpoint["agentId"]
            if agent_id in self.agents:
                agent = self.agents[agent_id]
                other_pending = [c for c in self.checkpoints.values()
                                if c["agentId"] == agent_id and c["status"] == "pending" and c["id"] != checkpoint_id]
