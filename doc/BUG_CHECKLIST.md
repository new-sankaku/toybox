...
| 「今の行」を示すhighlightが2つの理由で断続的に消えた。(1) 状態classが単一class(`.vd-seg-active` = 詳細度 0,1,0)なのに hover が `.vd-seg:hover`(0,2,0)で、**hoverの方が詳細度で勝ち**、今の行にcursorが乗った瞬間だけ帯が消える(追従scrollで今の行がcursorの下へ来るので頻発)。(2) 位置→行の対応に**隙間がある**ことを設計が織り込んでいなかった — 文字起こしのsegmentは発話の終端を持つので発話間(無音)はどの行にも属さず、実データで再生時間の3〜5割・1録画あたり数百〜3000箇所(録画965: 4935文中3064箇所・39.9%)に達し、一文ごとに点滅する。**状態を色で名乗るなら、hoverのような装飾より詳細度で勝たせ(2 class並記)、hoverは背景を失っても残る合図(box-shadow等)で別に置く。位置から行を引く対応は「どの行でもない時間」の割合を実data で測ってから、そこで印を消すか直前へ残すかを決める** | ×1 |
...
| 画面の既定表示(語なしの録画一覧)を描く経路がevent handler内にしか無く、起動時に誰も呼ばないため初回表示が「検索語を入力してください」のまま止まり、当たる語を先に発明しないと1本も開けない導線になっていた（runSearch(true)を起動時にも1回呼ぶ。既定状態は「操作の結果」ではないので、handler経由でしか作れない作りにしない） | ×1 |
| 配信中の解像度変動でffmpegがfilter graph全体を作り直す(入力option `reinit_filter` 既定1)ため、指定枚数を貯めてから1枚出す蓄積型の `tile` filterが作り直しのたびに貯めたframeを捨て、`-frames:v 1` が「最後の変更以降」だけのsprite sheetを掴む。頭の(最後の変更時刻÷interval)枚が丸ごと抜けて全体が前へ詰まり(実測3.68時間の録画でtile *i* に (i+55)*45秒 の絵)、足りない末尾55枚はEOF flushで黒く埋まる。seek barのhover画像が実際の動画と一致しないのに、sheetの寸法もtile数もdurationも正常なのでcacheは有効と判定され残り続ける。前段に `scale` を置いても防げない(作り直しは入力側で起きるためscaleへ届く前にresetされる)。修正: `-reinit_filter 0` を入力optionに追加し、cache鍵(_signature)へ判定条件を足して既存の壊れたspriteを無効化。※同種のchainでも `scale`+`fps` のような非蓄積型は1 frame程度しかずれないので、危ないのは状態を跨いで貯めるfilter | ×1 |
| 収集済みdataを検索indexへ均す経路(comment index)が **server起動時のbackfill 1箇所にしか無く**、稼働中に始まって終わった録画は次の再起動までcommentが0件のままだった(実測 2026-07-29、録画00398/00399: eventは703件・780件揃っているのにplayerのcomment panelも横断検索も空)。転写側はSTT完了時に index_transcript を呼ぶため症状はcommentにだけ出て、原因がdata欠落・軸ズレに見える。手動の張り直しscriptは既存hitとのdriftで判定するのでhit 0件の録画を素通りし、回収にも使えない。修正: 録画確定callback(_on_recording_finalized)でupdate_recordingの直後にindex_comments を張る(対象は起動時backfillと同じ completed のみ／失敗しても確定処理は止めない)。**「起動時に一度均す」だけの補完は、稼働中に生まれたdataを永久に取りこぼす** | ×1 |
| 意味検索(dense retrieval)が **秒をindexへ焼き付けたまま返す**ため、文字起こしのやり直し(media軸への貼り直し)やcomment indexの張り直しでsearch_hits側の秒だけが動くと、意味検索だけが構築時の古い時間軸を名乗り続ける(実測 331 group中134 groupが取り残され、中央値167秒・最大1,194秒=約20分ズレ。passageの48%が既に存在しないhit_idを指していた)。語で一致(FTS)はsearch_hitsを毎回読むため無事で、**症状が検索modeにだけ出る**ので原因がindex側だと見えにくい。差分buildの要否判定が件数と最大idだけなのも穴で、秒をUPDATEするだけの移行scriptでは永久に検出されない。修正: indexが持つのはvector・本文・hit_idだけとし、位置はhit_id経由でsearch_hitsから都度引く。indexが指す行が消えたpassageは古い秒で返さず結果から落として件数をuserへ提示し、index_statusに「張り直し後で除外中／未index」の件数を出して更新を促す。**派生indexへ「後から動く値」を複製したら、複製元の変更を検出する術か、複製をやめる設計のどちらかが要る** | ×1 |
| 相手が居る状態を1 eventから判定する経路(コラボ窓)が、切断content(`finish_content`)の**当事者を確認せず**自分の切断として窓を閉じていた。名乗っているのは`owner:{room_id, uid}`=終了したroomで、groupコラボでは他の参加者のroomが入る。他人が抜けるたびに自分の窓が閉じ、再開は次のroster snapshot待ちなので数十〜数百秒の穴になる(録画18本の照合で取りこぼし1586秒=最大成分)。同じ修正で「終端を名乗れない未クローズ窓は保存しない」判断も見直した — 「最後まで繋いだまま終わる配信は無い」という前提が実測で崩れていた(3録画・648秒)のに、判断だけがcodeに残っていた。修正: 名指しが自分なら切断、他人なら**その相手を外して残りが居ればまだ接続**と読み、session終了時は終了時刻で閉じて`closed_by`で終端の出所を名乗る。**「誰かが抜けた」eventを「自分が抜けた」と読まないこと。前提を書いたcommentは、前提が実測で崩れた時に誰も見に来ない** | ×1 |
| 収集dataを録画映像で検証するscriptが `ffprobe` の `pts_time` を録画のmedia軸として扱い、幻jump segmentを除いたcurated playlistでは軸が除外EXTINFぶん(実測264秒・2090秒)ずれるため、**正しいdataを「過大計上」と誤判定**した(該当録画で+11.6%。`-ss`で抜いた実フレームはDBと一致)。同じscriptがaspectだけでコラボとBattleを区別しようとしていたのも誤検出源(Battleも分割画面)。**検証の道具が壊れていると、健全なdataに不具合の診断が付く。突合が合わない時はまず測り方(時間軸・分類の解像度)を疑い、成果物そのもの(フレーム)で裏を取る** | ×1 |
