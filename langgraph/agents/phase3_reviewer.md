...
Reviewer Agentは以下のループで各レビュー観点を評価します：
...
| ステップ | 処理内容 | 成果物 |
|---------|---------|-------|
| 1. 収集 | 全Phase成果物を取得 | `review_materials` |
| 2. 観点作成 | レビュー観点をリスト化 | `review_aspects[]` |
| 3. 観点選択 | 次の観点を取得 | `current_aspect` |
| 4. 評価 | 基準に照らして評価 | `aspect_evaluation` |
| 5. 記録 | 問題と提案を記録 | `issues[]`, `suggestions[]` |
| 6. 次へ | 残観点があれば継続 | `pending_aspects[]` |
| 7. 総合スコア | 全観点を統合しスコア算出 | `overall_score` |
| 8. リスク評価 | ブロッカーとリスク分析 | `risk_assessment` |
| 9. 判定 | リリース可否を決定 | `release_decision` |

### スコア算出ロジック

```typescript
function calculateOverallScore(reviews: AspectReviews): number {
  const weights = {
    code_quality: 0.25,      // 25%
    asset_quality: 0.15,     // 15%
    gameplay: 0.15,          // 15%
    specification: 0.30,     // 30%
    performance: 0.15,       // 15%
  };

  let score = 0;
  for (const [aspect, weight] of Object.entries(weights)) {
    score += reviews[aspect].score * weight * 10;
  }
...
```typescript
interface IssueSeverityDecision {
  critical: {
    条件: "ゲームプレイ不能 or データ損失リスク";
    対応: "即座にブロッカー登録";
  };
  major: {
    条件: "主要機能に影響、回避策なし";
...
Reviewerはリトライせず、発見した問題を全て記録：
- 問題が多すぎる場合: REJECTED判定
- 判断困難な問題: Human確認を要求（`interrupt()`）
- 最終リリース判定: 必ずHuman確認
