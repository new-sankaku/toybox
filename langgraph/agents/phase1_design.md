...
Design Agentは以下のループで各コンポーネントを設計します：
...
| ステップ | 処理内容 | 成果物 |
|---------|---------|-------|
| 1. リスト作成 | 必要コンポーネントを洗い出し | `pending_components[]` |
| 2. 選択 | 依存関係順に次を取得 | `current_component` |
| 3. 責務定義 | 単一責任の原則で責務明確化 | `responsibility` |
| 4. 依存関係 | 入出力と連携先を定義 | `dependencies` |
| 5. 型定義 | TypeScript interfaceを設計 | `type_definitions` |
| 6. 次へ | 完了リストに追加 | `completed_components[]` |
| 7. 検証 | 全体の依存グラフを検証 | `DesignOutput` |

### 品質チェック基準

各コンポーネントは以下を満たすまでループ：

```typescript
interface ComponentValidation {
  // SOLID原則チェック
  single_responsibility: boolean;  // 責務が1つに絞られているか
  open_closed: boolean;           // 拡張に開いているか
  dependency_inversion: boolean;  // 抽象に依存しているか

  // 実装可能性
  has_clear_interface: boolean;   // インターフェースが明確か
  no_circular_deps: boolean;      // 循環依存がないか
  testable: boolean;              // テスト可能か
...
```
1. Core層（依存なし）
   └─ GameConfig, Constants, Types

2. Infrastructure層
   └─ EventBus, StateManager, AssetLoader
