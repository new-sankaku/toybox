...
World Agentは以下のループで各ロケーション/要素を構築します：
...
| ステップ | 処理内容 | 成果物 |
|---------|---------|-------|
| 1. 世界の骨格 | 基本ルールと構造を定義 | `world_rules`, `factions` |
| 2. リスト作成 | 必要ロケーションを洗い出し | `pending_locations[]` |
| 3. ロケ選択 | 重要度順に次を取得 | `current_location` |
| 4. 基本設定 | 名称、位置、雰囲気を定義 | `basic_info` |
| 5. 詳細設計 | 視覚・住人・歴史を追加 | `details` |
| 6. ゲームプレイ | 機能とイベントを設計 | `gameplay_elements` |
| 7. 接続設計 | 他エリアとの繋がり定義 | `connections` |
| 8. 次へ | 完了リストに追加 | `completed_locations[]` |
| 9. マップ検証 | 全体の整合性確認 | `WorldOutput` |
...
interface LocationValidation {
  // アイデンティティ
  has_unique_identity: boolean;    // 独自の個性があるか
  visually_distinct: boolean;      // 視覚的に区別できるか
  fits_world_theme: boolean;       // 世界観と一致するか

  // 機能性
  has_gameplay_purpose: boolean;   // ゲームプレイ上の役割があるか
  has_exploration_value: boolean;  // 探索する価値があるか
  supports_story: boolean;         // ストーリーをサポートするか

  // 接続性
  is_reachable: boolean;           // 到達可能か
  connections_logical: boolean;    // 接続が論理的か

  // 詳細度
  enough_detail_for_assets: boolean; // アセット制作に十分な情報か
