...
Concept Agentは以下のループで各コンセプト要素を精錬します：
...
| ステップ | 処理内容 | 成果物 |
|---------|---------|-------|
| 1. 要素リスト作成 | 定義が必要な要素を洗い出し | `pending_elements[]` |
| 2. 要素選択 | 優先度順に次の要素を取得 | `current_element` |
| 3. 初期案生成 | 3つの方向性を提示 | `draft_options[3]` |
| 4. 詳細化 | 選択案に具体性を追加 | `detailed_element` |
| 5. 整合性チェック | 他要素・制約との整合確認 | `validation_result` |
| 6. 次の要素へ | 完了済みに移動、次を取得 | `completed_elements[]` |
| 7. 全体統合 | 全要素の最終調整 | `ConceptOutput` |
...
interface ElementValidation {
  // 必須チェック
  is_specific: boolean;      // 曖昧でなく具体的か
  is_measurable: boolean;    // 成功基準が測定可能か
  is_achievable: boolean;    // 制約内で実現可能か
  is_consistent: boolean;    // 他要素と矛盾しないか
