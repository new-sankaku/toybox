...
配信ごとに音量が大きくばらつくため、clip・焼き込み出力・Up出力のどれから出しても同じ
目標値へ揃うようにする。目標はEBU R128のI(統合ラウドネス)とTP(true peak)の2値だけで、
...
def audio_filter(target_lufs: float, true_peak: float) -> str:
    return f"aresample=async=1,loudnorm=I={target_lufs:g}:TP={true_peak:g}"


def encode_args(target_lufs: float, true_peak: float, bitrate_kbps: int,
                sample_rate: Optional[int]) -> list:
    ...
    args = ["-c:a", "aac", "-b:a", f"{int(bitrate_kbps)}k",
            "-af", audio_filter(target_lufs, true_peak)]
