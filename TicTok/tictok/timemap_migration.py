...
母集団は2つに分かれている:
  ...
  * 幻のtimestamp jumpを踏んだもの … 版2の ``_rebase_phantom_jumps`` が畳むので出力が変わる。
    実測(録画00126)は2時間51分の素材に対し転写の終端が10時間43分で、字幕は録画の外側を指す。
...
無くなる。そこで素材の実尺で選別する — 転写の尺が素材に収まっていれば畳む対象は無かった
のだから昇格させ、食い違うものだけ版1のまま残して「要再転写」を名乗らせる。

物差しは焼き込みの関門と同じ ``video_overlay.material_media_seconds`` と
``subtitles.axis_matches_media`` を使う。ここで判定を書き写すと、同じ転写が経路によって
合格したり弾かれたりする。

実尺が測れない録画(素材が消えた旧録画)は昇格させる。``axis_matches_media`` と同じ立場で、
降格は「ズレている証拠がある」ときにだけ行う — 根拠が無いまま版1へ据え置くと、正しい転写
まで焼き込めなくなる。
...
# 上げないと、新しい物差しなら合格するはずの転写が古い判定のまま据え置かれる。
#
