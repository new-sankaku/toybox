...
SETTING_DEFS = {
    ...
    "transcribe_sweep_enabled": {
        ...
        "min": 0,
        "max": 1,
        "label": "文字起こし: 起動時に文字起こしのない録画をすべてqueueへ積む",
        "note": "本数で区切らないのは、文字起こしがGPUを1本ずつ直列に使い、queue(Job画面の種別「文字起こし」)が再起動をまたいで残るためです。全部積んでも同時に走るのは常に1本で、終わらなかったぶんは次の起動でそのまま続きます。文字起こし済みの録画と、既に待機・実行中の録画は積み直しません(積むのは文字起こしのないものだけです)。1本あたり数十分かかるので、貯まっている録画が多いと数日ぶんのGPU作業がqueueに並びます。0でこの自動処理を行いません。",
    },
