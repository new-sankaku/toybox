...
SETTING_DEFS = {
    ...
    "message_dedup_window": {
        ...
        "note": "TikTokは接続のたびに、そのRoomの直近Commentを遡って送り直します。配信の開始時と切断からの復帰時に接続が起きるため、この遡り分を落とさないと同じCommentが接続回数だけ記録されます。ここで指定した秒数のあいだ受信済みのmessage IDを覚えておき、届き直したものを捨てます。短すぎると、切断が長引いた後の復帰で遡り分を落とし損ねます。長くしても記録は増えず、覚えておくmessage IDの数だけメモリを使います。",
    },
    ...
    "opponent_room_retry_seconds": {
        ...
        "note": "相手RoomへのGift取得接続に失敗したとき、この間隔をあけて繋ぎ直します。失敗の多くはsign serverの一時的な5xxで、同じ相手でも時刻を変えれば繋がります(実測: 500を受けた配信者80人のうち66人は別のPKでは取得できていました)。繋ぎ直しが遅れたぶんのGiftは取り戻せないため、短いほど取りこぼしは減りますが、TikTokへの接続試行は増えます。0にすると再試行しません。",
    },
    "opponent_room_retry_max": {
        ...
        "note": "この回数だけ続けて失敗したら、その相手への接続を諦めます。相手が配信を終えた室は何度撃っても通らないため、繋がらない相手を撃ち続けないための上限です。間隔は失敗のたびに伸び60秒で頭打ちになるので、既定(15秒間隔・8回)なら累計約390秒 — PK1戦(約301秒)を撃ち切ります。諦めた時点はlogに残り、その相手の実弾は「未取得」と表示されます。",
    },
    "opponent_room_keep_seconds": {
        ...
        "note": "PKが終わっても、同じ相手とのコラボが続いている間と、この秒数のあいだは接続を保ちます。コラボ中は同じ相手と連戦するため、PKごとに繋ぎ直すと接続と確認だけが増えます(実測: 延べ接続の55%が同一配信・同一相手への張り直しで、前のPK終了からの間隔は中央値104秒)。0にするとPK終了ごとに切断します。",
    },
    ...
    "playback_gain_enabled": {
        ...
        "max": 1,
        "label": "再生時に音量を揃える",
        "note": "再生画面で、上の目標ラウドネスへ向けて音量を揃えて鳴らします。録画のfileは書き換えず、あらかじめ測っておいたgainの時系列を再生時に当てるだけなので、素の音はいつでも残っています(再生画面のcheckboxでその場で切り替えられます)。曲線の無い録画は素のまま鳴ります。",
        "options": [
            {"value": 0, "label": "しない"},
            {"value": 1, "label": "する"},
        ],
    ...
    "playback_gain_max_boost_db": {
        ...
        "note": "静かな区間をどこまで持ち上げてよいかの上限です。上限を外すと、息継ぎや暗騒音まで目標へ持ち上がって耳につきます。この上限とは別に、持ち上げた結果が下の上限ピークを超えないところで必ず頭が押さえられます。",
    },
    "playback_gain_max_cut_db": {
        ...
        "max": 30.0,
        "label": "再生時に下げる上限（dB）",
        "note": "大きい区間をどこまで下げてよいかの上限です。上限ピークを守るための引き下げはこの値に縛られません(音が割れる方が害が大きいため)。",
    },
    ...
    "gain_sweep_per_start": {
        ...
        "note": "配信ごと・場面ごとの音量差を再生時に揃えるための、当てるべき音量(gain)の時系列です。録画そのものは1 byteも書き換えず、曲線だけをsidecarへ持ちます。作られていない録画では音量が揃わないまま再生されます。sweep(起動時と、下の「繰り返す間隔」ごと)1回につきこの本数だけqueueへ積みます。音声だけをdecodeして測る処理でGPUは使わず、実測で2.9時間の録画に19秒です。0でこの自動処理を行いません。",
    },
    ...
    "semantic_sweep_enabled": {
        ...
        "note": "AIによる意味検索が読むindexを、上の自動生成と同じsweepで作り直します。indexの材料は文字起こしとcommentの検索行なので、録画が確定した時点ではまだ材料が揃っておらず、文字起こしが終わった後の周期で初めて対象になります(差分だけを埋め込むので、同じ録画を作り直すことはありません)。埋め込みserverが無効・未設定のとき、未indexが0件のとき、既に構築中のときは何もしません。埋め込みserverへ届かず失敗した場合は、同じ失敗が台帳へ積み上がらないよう1時間は自動で再試行しません(検索画面の「indexを更新」はいつでも押せます)。0でこの自動処理を行いません。",
    },
