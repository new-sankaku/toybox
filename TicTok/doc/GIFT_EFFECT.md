...
## 5. そもそも「正解動画」が存在しない演出がある

TikTokのgift演出は、Effect Houseの **LIVE Gift** プログラムで外部制作者が作っている。TikTok
純正とサード製の区別は無い。3種類あり、そのうち1つが決定的である。

| 種別 | 内容 |
|---|---|
| Appreciation Gift | 感謝を表現する |
| **Performance-Amplifying Gift** | **配信者の動きに同期する** |
| LIVE Stream-Enhancing Gift | 配信を盛り上げる |

**配信者の動きに同期する**ということは、実行時入力に依存して絵が変わるということで、
**その演出の「正解動画」が原理的に存在しない**。asset経路が仮に全部通っても、この種別は
動画にならない。

裏返すと、**描かれた結果を画面から採る以外に動画を得る道が無い**。
...
3. effect endpointの実URL。4章により価値が無いので追わない
