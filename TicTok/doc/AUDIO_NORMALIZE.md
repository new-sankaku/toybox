...
この文書が扱うのは**出力を作るとき**と**mp4そのものを差し替えるとき**の正規化である。
再生画面で音量が揃うのは、録画を書き換えずgainの時系列だけを当てる別経路で、実装も設定の
一部も分かれている（[PLAYBACK_GAIN.md](PLAYBACK_GAIN.md)）。目標ラウドネス
(`audio_normalize_lufs`)と天井(`audio_normalize_true_peak`)だけは両者で共有する — 同じ
「どこへ揃えるか」を経路ごとに別の値で持つと、再生で聞いた音と出力した音の高さが食い違う。
