...
**笑いhitの97.2%が2配信者（streamer_a / streamer_c）のもの**で、他5名は合計222件。
...
既定値では既存挙動と完全に同一。当時は配信者pageの見どころ（`streamer_highlights`）にも
影響しないことが条件だったが、その節は後に画面ごと廃止した（`core/spike.py` を使うのは
録画単位の切り抜き候補だけになっている）。
