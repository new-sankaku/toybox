...
def register_metrics_routes(app: Flask, data_store: DataStore, sio=None):
    ...
    def get_ai_request_stats(project_id: str):
        ...
        return jsonify(
            {"total": total, "processing": processing, "pending": pending, "completed": completed, "failed": failed}
        )
    ...
    def update_project_asset(project_id: str, asset_id: str):
        ...
        if sio:
            sio.emit("asset:updated", {"projectId": project_id, "asset": asset}, room=f"project:{project_id}")
