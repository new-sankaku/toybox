...
def register_quality_settings_routes(app:Flask,project_service,workflow_service):
 ...
 def get_quality_settings(project_id:str):
  ...
  settings=workflow_service.get_quality_settings(project_id)
  settings_dict={}
 ...
 def update_quality_setting(project_id:str,agent_type:str):
  ...
  current_settings=workflow_service.get_quality_settings(project_id)
  ...
  workflow_service.set_quality_setting(project_id,agent_type,updated_config)
  return jsonify({
 ...
 def reset_quality_settings(project_id:str):
  ...
  if not project:
   return jsonify({"error":"Project not found"}),404
  default_settings=get_default_quality_settings()
  workflow_service.reset_quality_settings(project_id)
  settings_dict={}
  for agent_type,config in default_settings.items():
   settings_dict[agent_type]=config.to_dict()
  return jsonify({
