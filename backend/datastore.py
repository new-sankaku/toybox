...
class DataStore:
    ...
    def _start_agent(self, session, agent_dict: Dict):
        ...
        self._emit_event(
            "agent:started",
            {"agentId": agent.id, "projectId": agent.project_id, "agent": agent_repo.to_dict(agent)},
            agent.project_id,
        )
        ...
        for point_progress, asset_type, asset_name, asset_size in asset_points:
            if old_progress < point_progress <= new_progress:
                ...
                existing = [
                    ...
                    and a["agent"]
                    == (agent.metadata_.get("displayName", agent.type) if agent.metadata_ else agent.type)
                ]
                if not existing:
...
"""
    ...
    def _create_asset(self, session, agent, asset_type: str, name: str, size: str):
        ...
        self._emit_event(
            "asset:created",
            {"projectId": agent.project_id, "asset": asset_repo.to_dict(asset), "autoApproved": auto_approve},
            agent.project_id,
        )
    ...
    def _update_project_metrics(self, session, project_id: str):
        ...
        metrics_data = {
            ...
            "estimatedEndTime": (datetime.now() + timedelta(seconds=estimated_remaining)).isoformat()
        ...
        with session_scope() as session:
            ...
            for data in agents_data:
                agent_repo.create_from_dict(
                    project_id,
                    {"type": data["type"], "phase": data["phase"], "metadata": {"displayName": data["name"]}},
                )
        ...
        with session_scope() as session:
            ...
            if agent:
                if resolution == "rejected":
                    ...
                    session.flush()
                    self._add_system_log_internal(
                        session, project_id, "warn", "System", f"{agent.type}が却下されました"
                    )
                    self._emit_event(
                        "agent:failed", {"agentId": agent.id, "projectId": project_id, "reason": "rejected"}, project_id
                    )
                    ...
                    if not other_pending and agent.status == "waiting_approval":
                        ...
                        session.flush()
                        self._add_system_log_internal(
                            session, project_id, "info", "System", f"{agent.type}が承認されました"
                        )
                        self._emit_event(
                            "agent:completed",
                            {
    ...
    def create_intervention(
        ...
        with session_scope() as session:
            ...
            self._add_system_log_internal(
                session, project_id, "info", "Human", f"[{priority_desc}] {target_desc}への介入: {message[:50]}..."
            )
            return intervention
    ...
    def delete_intervention(self, intervention_id: str) -> bool:
        with session_scope() as session:
            ...
            if result:
                self._add_system_log_internal(
                    session, project_id, "info", "System", f"介入削除: {intervention.message[:30]}..."
                )
            return result
    ...
    def activate_agent_for_intervention(self, agent_id: str, intervention_id: str) -> Dict:
        with session_scope() as session:
            ...
            if agent.status in activatable_statuses:
                ...
                session.flush()
                syslog_repo.add_log(
                    agent.project_id, "info", "System", f"エージェント {display_name} が返答を受けて再開"
                )
                return {
                    "activated": True,
                    "agent": agent_repo.to_dict(agent),
    ...
    def _pause_subsequent_agents(self, session, target_agent) -> List[Dict]:
        ...
        for agent_dict in agents:
            ...
            if agent_phase > target_phase and agent_dict["status"] == "running":
                ...
                display_name = agent.metadata_.get("displayName", agent.type) if agent.metadata_ else agent.type
                syslog_repo.add_log(
                    agent.project_id, "info", "System", f"エージェント {display_name} を後続フェーズとして一時停止"
                )
    ...
    def add_intervention_response(
        self, intervention_id: str, sender: str, message: str, agent_id: str = None
    ) -> Optional[Dict]:
        with session_scope() as session:
            intervention_repo = InterventionRepository(session)
            agent_repo = AgentRepository(session)
        ...
        with session_scope() as session:
            ...
            if intervention.target_agent_id:
                ...
                if agent and agent.status == "waiting_response":
                    ...
                    display_name = agent.metadata_.get("displayName", agent.type) if agent.metadata_ else agent.type
                    syslog_repo.add_log(
                        intervention.project_id, "info", "System", f"エージェント {display_name} が返答を受けて再開"
                    )
            if result:
    ...
    def create_uploaded_file(
        ...
        with session_scope() as session:
            ...
            self._add_system_log_internal(
                session, project_id, "info", "Upload", f"ファイルアップロード: {original_filename} ({category})"
            )
            return uf
    ...
    def _auto_approve_pending_checkpoints(self, session, project_id: str, rules: List[Dict]):
        ...
        pending_cps = (
            session.query(Checkpoint).filter(Checkpoint.project_id == project_id, Checkpoint.status == "pending").all()
        )
        cp_repo = CheckpointRepository(session)
        agent_repo = AgentRepository(session)
        now = datetime.now()
    ...
    def _auto_approve_pending_assets(self, session, project_id: str, rules: List[Dict]):
        ...
        pending_assets = (
            session.query(Asset).filter(Asset.project_id == project_id, Asset.approval_status == "pending").all()
        )
        asset_repo = AssetRepository(session)
        for asset in pending_assets:
            if asset.type in enabled_categories:
                ...
                session.flush()
                self._add_system_log_internal(
                    session, project_id, "info", "System", f"アセット自動承認(設定変更): {asset.name}"
                )
                self._emit_event(
                    "asset:updated",
                    {"projectId": project_id, "asset": asset_repo.to_dict(asset), "autoApproved": True},
                    project_id,
                )
    ...
    def retry_agent(self, agent_id: str) -> Optional[Dict]:
        ...
        with session_scope() as session:
            ...
            self._emit_event(
                "agent:retry",
                {"agentId": agent.id, "projectId": agent.project_id, "agent": result, "previousStatus": old_status},
                agent.project_id,
            )
            return result
    ...
    def pause_agent(self, agent_id: str) -> Optional[Dict]:
        with session_scope() as session:
            ...
            display_name = agent.metadata_.get("displayName", agent.type) if agent.metadata_ else agent.type
            syslog_repo.add_log(
                agent.project_id, "info", "System", f"エージェント {display_name} を一時停止（前状態: {old_status}）"
            )
            result = agent_repo.to_dict(agent)
            return result
    ...
    def resume_agent(self, agent_id: str) -> Optional[Dict]:
        with session_scope() as session:
            ...
            display_name = agent.metadata_.get("displayName", agent.type) if agent.metadata_ else agent.type
            syslog_repo.add_log(
                agent.project_id, "info", "System", f"エージェント {display_name} を再開（前状態: {old_status}）"
            )
            result = agent_repo.to_dict(agent)
            return result
    ...
    def complete_trace(
        ...
        with session_scope() as session:
            ...
            return repo.complete_trace(
                trace_id, llm_response, output_data, tokens_input, tokens_output, status, output_summary=output_summary
            )
