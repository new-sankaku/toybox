...
class AgentExecutionService:
    ...
    async def execute_agent(self, project_id: str, agent_id: str) -> Dict[str, Any]:
        ...
        if not self._can_start_agent(project_id, agent["type"]):
            self._logger.info(
                f"execute_agent: dependencies not met: agent_type={agent['type']} project_id={project_id}"
            )
        ...
        self._data_store.update_agent(
            agent_id,
            {"status": "running", "progress": 0, "startedAt": datetime.now().isoformat(), "currentTask": "初期化中"},
        )
        self._emit_event(
            "agent:started",
            {"agentId": agent_id, "projectId": project_id, "agent": self._data_store.get_agent(agent_id)},
            project_id,
        )
        ...
        try:
            ...
            if output.status == AgentStatus.COMPLETED:
                ...
                self._emit_event(
                    "agent:failed", {"agentId": agent_id, "projectId": project_id, "error": output.error}, project_id
                )
                return {"success": False, "error": output.error}
        except TokenBudgetExceededError as e:
            self._logger.warning(
                f"token budget exceeded: agent_id={agent_id} project_id={project_id} used={e.used} limit={e.limit}"
            )
            self._data_store.update_agent(
                agent_id,
                {
            ...
            self._logger.error(
                f"execute_agent failed: agent_id={agent_id} project_id={project_id} error={e}", exc_info=True
            )
            self._data_store.update_agent(
                agent_id,
                {
            ...
            self._emit_event(
                "agent:failed", {"agentId": agent_id, "projectId": project_id, "error": str(e)}, project_id
            )
            return {"success": False, "error": str(e)}
        finally:
            with self._lock:
    ...
    async def execute_leader_with_workers(self, project_id: str, leader_agent_id: str) -> Dict[str, Any]:
        if not self._agent_runner:
            return {"success": False, "error": "Agent runner not configured"}
        ...
        self._emit_event(
            "agent:started",
            {"agentId": leader_agent_id, "projectId": project_id, "agent": self._data_store.get_agent(leader_agent_id)},
            project_id,
        )
        context = AgentContext(
            project_id=project_id,
        ...
        try:
            ...
            self._logger.error(
                f"execute_leader_with_workers failed: leader_id={leader_agent_id} project_id={project_id} error={e}",
                exc_info=True,
            )
            self._data_store.update_agent(
                leader_agent_id,
            ...
            self._emit_event(
                "agent:failed", {"agentId": leader_agent_id, "projectId": project_id, "error": str(e)}, project_id
            )
            return {"success": False, "error": str(e)}
        finally:
            with self._lock:
    ...
    def _on_checkpoint(self, agent_id: str, project_id: str, cp_type: str, data: Dict) -> None:
        self._data_store.create_checkpoint(
            project_id,
            agent_id,
            {
                "type": cp_type,
    ...
    def _on_worker_created(self, project_id: str, parent_agent_id: str, worker_type: str, task: str) -> str:
        ...
        self._emit_event(
            "agent:created",
            {"agentId": worker_id, "projectId": project_id, "parentAgentId": parent_agent_id, "agent": worker},
            project_id,
        )
        return worker_id
    ...
    def _on_worker_status(self, project_id: str, worker_id: str, status: str, data: Dict) -> None:
        ...
        self._emit_event(
            f"agent:{status}",
            {"agentId": worker_id, "projectId": project_id, "agent": self._data_store.get_agent(worker_id)},
            project_id,
        )
        ...
        with self._lock:
            if agent_id in self._running_agents:
                ...
                if agent:
                    ...
                    self._emit_event(
                        "agent:failed",
                        {"agentId": agent_id, "projectId": project_id, "error": "キャンセルされました"},
                        project_id,
                    )
                return True
        return False
    ...
    async def re_execute_agent(self, project_id: str, agent_id: str) -> None:
        try:
            ...
            if not agent:
                ...
                return
            self._logger.info(
                f"re_execute_agent: starting agent_id={agent_id} type={agent.get('type', '')} project_id={project_id}"
            )
