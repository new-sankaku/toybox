...
class ArchiveService:
    ...
    def cleanup_completed_project_data(self, project_id: str, keep_recent_days: int = 7) -> Dict[str, int]:
        ...
        with session_scope() as session:
            deleted_counts["traces"] = (
                ...
                .filter(AgentTrace.project_id == project_id, AgentTrace.started_at < cutoff_date)
            ...
            if agent_ids:
                deleted_counts["agent_logs"] = (
                    ...
                    .filter(AgentLog.agent_id.in_(agent_ids), AgentLog.created_at < cutoff_date)
                    .delete(synchronize_session=False)
                )
            session.commit()
        total = sum(deleted_counts.values())
    ...
    def export_traces_to_zip(
        self, project_id: str, agent_id: Optional[str] = None, include_logs: bool = True
    ) -> Optional[str]:
    ...
    def archive_and_cleanup(self, project_id: str, agent_id: Optional[str] = None) -> Dict[str, any]:
        ...
        with session_scope() as session:
            ...
            if agent_id:
                deleted["agent_logs"] = (
                    session.query(AgentLog).filter(AgentLog.agent_id == agent_id).delete(synchronize_session=False)
                )
            session.commit()
        return {
            "success": True,
    ...
    def archive_old_traces(self, days_old: Optional[int] = None) -> Dict[str, any]:
        ...
        with session_scope() as session:
            ...
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED, compresslevel=9) as zf:
                ...
                zf.writestr("metadata.json", json.dumps(metadata, ensure_ascii=False, indent=2))
            deleted = (
                session.query(AgentTrace).filter(AgentTrace.started_at < cutoff_date).delete(synchronize_session=False)
            )
            session.commit()
        return {
            "success": True,
