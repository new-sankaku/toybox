...
class LocalAudioCraftProvider(AIProvider):
    ...
    def test_connection(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(f"{base_url}/api/health", timeout=self.config.timeout)
            if response.status_code == 200:
                return {"success": True, "message": "AudioCraft接続成功"}
            response = requests.get(base_url, timeout=self.config.timeout)
            if response.status_code == 200:
                return {"success": True, "message": "AudioCraft接続成功 (Gradio UI)"}
            return {"success": False, "message": f"AudioCraft接続失敗: HTTP {response.status_code}"}
        except requests.exceptions.ConnectionError:
            return {"success": False, "message": f"AudioCraftに接続できません ({base_url})"}
        except requests.exceptions.Timeout:
            return {"success": False, "message": "AudioCraft接続タイムアウト"}
        except Exception as e:
            return {"success": False, "message": f"AudioCraft接続エラー: {str(e)}"}
    ...
    def generate_audio(
        ...
        **kwargs,
        ...
        try:
            ...
            return {"success": False, "message": f"音声生成失敗: HTTP {response.status_code}"}
        except requests.exceptions.Timeout:
            return {"success": False, "message": "音声生成タイムアウト（長い音声の場合は時間がかかります）"}
        except Exception as e:
            return {"success": False, "message": f"音声生成エラー: {str(e)}"}

    def generate_music(
        self, prompt: str, duration: float = 30.0, model: str = "musicgen-medium", **kwargs
    ) -> Dict[str, Any]:
        return self.generate_audio(prompt=prompt, model=model, duration=duration, **kwargs)

    def generate_sfx(self, prompt: str, duration: float = 5.0, **kwargs) -> Dict[str, Any]:
