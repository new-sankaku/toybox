...
class ClaudeCodeProvider(AIProvider):
    ...
    def _get_git_changes(self, working_dir: str) -> List[Dict[str, str]]:
        try:
            result = subprocess.run(
                ["git", "diff", "--name-status", "HEAD"], cwd=working_dir, capture_output=True, text=True, timeout=10
            )
            changes = []
            for line in result.stdout.strip().split("\n"):
                if line:
                    ...
                    if len(parts) == 2:
                        ...
                        changes.append({"path": path, "action": action_map.get(status[0], "unknown")})
            return changes
    ...
    def chat(
        ...
        try:
            result = subprocess.run(
                cmd, cwd=working_dir, capture_output=True, text=True, timeout=kwargs.get("timeout", 600)
            )
    ...
    def test_connection(self) -> Dict[str, Any]:
        try:
            ...
            if result.returncode == 0:
                ...
                return {"success": True, "message": f"Claude Code CLI: {version}"}
            return {"success": False, "message": f"Claude Code CLI error: {result.stderr}"}
        except FileNotFoundError:
    ...
    def execute_action(
        ...
        try:
            ...
            return {"status": "completed", "content": response.content, "changed_files": [], "exit_code": 0}
