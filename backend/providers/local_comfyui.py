...
class LocalComfyUIProvider(AIProvider):
 ...
 def test_connection(self)->Dict[str,Any]:
  ...
  try:
   ...
   return {
    "success":False,
    "message":f"ComfyUI接続失敗: HTTP {response.status_code}"
   }
  except requests.exceptions.ConnectionError:
   return {
    "success":False,
    "message":f"ComfyUIに接続できません ({base_url})"
   }
  except requests.exceptions.Timeout:
   return {
    "success":False,
    "message":"ComfyUI接続タイムアウト"
   }
  except Exception as e:
   return {
    "success":False,
    "message":f"ComfyUI接続エラー: {str(e)}"
   }
  ...
  **kwargs
  ...
  try:
   ...
   return {
    "success":False,
    "message":f"画像生成リクエスト失敗: HTTP {response.status_code}"
   }
  except Exception as e:
   return {
    "success":False,
