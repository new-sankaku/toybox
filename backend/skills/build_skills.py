...
class CodeBuildSkill(Skill):
    ...
    parameters = [
        SkillParameter(
            name="build_type",
            type="string",
            description="ビルドタイプ: npm, yarn, python, godot, make",
            required=False,
            default="auto",
        ),
        SkillParameter(
            name="command",
            type="string",
            description="カスタムビルドコマンド（指定時はbuild_typeを無視）",
            required=False,
        ),
    ...
    async def execute(self, context: SkillContext, **kwargs) -> SkillResult:
        ...
        if result.success:
            return SkillResult(
                success=True, output=result.output, metadata={"command": command, "build_type": build_type}
            )
        return SkillResult(
            success=False,
...
class CodeTestSkill(Skill):
    ...
    parameters = [
        SkillParameter(
            name="test_type",
            type="string",
            description="テストタイプ: pytest, jest, npm, yarn, go, cargo",
            required=False,
            default="auto",
        ),
        ...
        SkillParameter(
            name="args", type="string", description="追加引数（例: -v, --coverage）", required=False, default=""
        ),
    ]
...
class CodeLintSkill(Skill):
    ...
    parameters = [
        SkillParameter(
            name="lint_type",
            type="string",
            description="リンタータイプ: eslint, ruff, flake8, pylint, prettier, black",
            required=False,
            default="auto",
        ),
    ...
    def _detect_lint_type(self, working_dir: str) -> Optional[str]:
        ...
        if os.path.exists(os.path.join(working_dir, "package.json")):
            return "eslint"
        py_files = any(
            f.endswith(".py") for f in os.listdir(working_dir) if os.path.isfile(os.path.join(working_dir, f))
        )
        if py_files:
            return "ruff"
        return None
