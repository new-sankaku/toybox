...
class CodeSearchSkill(FileSkillMixin,Skill):
    name="code_search"
    ...
    async def execute(self,context:SkillContext,**kwargs)->SkillResult:
        ...
        if fm:
            results=fm.search_content(pattern,file_pattern,case_sensitive,max_results,context_lines)
            if results or fm._get_cache().is_loaded:
                truncated=len(results)>=max_results
                return SkillResult(
                    success=True,
                    output=results,
                    metadata={"pattern":pattern,"total_matches":len(results),"truncated":truncated,"from_cache":True}
                )
        if path==".":
        ...
        try:
            ...
            truncated=len(results)>=max_results
            metadata={"pattern":pattern,"total_matches":len(results),"truncated":truncated,"max_results_applied":max_results,"from_cache":False}
            if skipped:
    ...
    def _search_file(self,filepath:str,regex:re.Pattern,context_lines:int,base_path:str)->List[Dict[str,Any]]:
        ...
        return results
class FileSearchSkill(FileSkillMixin,Skill):
    name="file_search"
    ...
    async def execute(self,context:SkillContext,**kwargs)->SkillResult:
        ...
        fm=self.get_file_manager()
        if fm:
            file_paths=fm.search_files(pattern,max_results)
            if file_paths or fm._get_cache().is_loaded:
                if not include_hidden:
                    file_paths=[p for p in file_paths if not os.path.basename(p).startswith(".")]
                all_files=fm.get_all_files()
                results=[]
                for p in file_paths[:max_results]:
                    info=all_files.get(p,{})
                    results.append({
                        "name":os.path.basename(p),
                        "path":p,
                        "size":info.get("size",0),
                        "modified":info.get("mtime",0),
                    })
                truncated=len(results)>=max_results
                return SkillResult(
                    success=True,
                    output=results,
                    metadata={"pattern":pattern,"total_found":len(results),"truncated":truncated,"from_cache":True}
                )
        if path==".":
            full_path=context.working_dir
        elif os.path.isabs(path):
        ...
        try:
            ...
            return SkillResult(
                success=True,
                output=results,
                metadata={"pattern":pattern,"total_found":len(results),"truncated":truncated,"max_results_applied":max_results,"from_cache":False}
            )
        except Exception as e:
            return SkillResult(success=False,error=str(e))
