...
class CodeSearchSkill(Skill):
    ...
    parameters = [
        ...
        SkillParameter(
            name="file_pattern",
            type="string",
            description="ファイルパターン（例: *.py, *.ts）",
            required=False,
            default="*",
        ),
        SkillParameter(
            name="case_sensitive", type="boolean", description="大文字小文字を区別", required=False, default=True
        ),
        ...
        try:
            ...
            if skipped:
                ...
                metadata["skipped_reason"] = (
                    f"{len(skipped)} file(s) skipped due to size limit ({max_search_file_size} bytes)"
                )
                ...
                try:
                    ...
                    if file_size > max_search_file_size:
                        ...
                        skipped_files.append(
                            {"file": rel_path, "size": file_size, "reason": f"exceeds {max_search_file_size} bytes"}
                        )
                        get_logger().debug(
                            f"Skipped large file during search {filepath}: {file_size} bytes > {max_search_file_size}"
                        )
                        continue
    ...
    def _search_file(
        self, filepath: str, regex: re.Pattern, context_lines: int, base_path: str
    ) -> List[Dict[str, Any]]:
        results = []
        try:
        ...
        for i, line in enumerate(lines):
            if regex.search(line):
                ...
                context = [
                    {"line_no": j + 1, "content": lines[j].rstrip(), "is_match": j == i} for j in range(start, end)
                ]
                results.append(
                    {
                        "file": rel_path,
...
class FileSearchSkill(Skill):
    ...
    parameters = [
        SkillParameter(
            name="pattern", type="string", description="ファイル名パターン（glob形式、例: *.py, test_*.ts）"
        ),
        SkillParameter(name="path", type="string", description="検索対象ディレクトリ", required=False, default="."),
        SkillParameter(name="max_results", type="integer", description="最大結果数", required=False, default=100),
        SkillParameter(
            name="include_hidden", type="boolean", description="隠しファイルを含める", required=False, default=False
        ),
    ]
