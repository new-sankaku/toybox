...
class FileReadSkill(Skill):
    ...
    async def execute(self, context: SkillContext, **kwargs) -> SkillResult:
        ...
        config_max_lines = context.restrictions.get("max_lines", 1000)
        max_lines = min(max_lines, config_max_lines)
        ...
        try:
            ...
            if allowed_ext:
                ...
                if ext and ext not in allowed_ext:
                    return SkillResult(
                        success=False, error=f"Extension not allowed: {ext}. Allowed: {', '.join(allowed_ext)}"
                    )
            ...
            if file_size > max_file_size:
                return SkillResult(
                    success=False,
                    error=f"File too large: {file_size} bytes (limit: {max_file_size} bytes). Use max_lines to read partial content, or split the operation.",
                )
...
class FileWriteSkill(Skill):
    ...
    parameters = [
        ...
        SkillParameter(name="encoding", type="string", description="エンコーディング", required=False, default="utf-8"),
        SkillParameter(
            name="create_dirs", type="boolean", description="親ディレクトリを作成するか", required=False, default=True
        ),
    ]
    ...
    async def execute(self, context: SkillContext, **kwargs) -> SkillResult:
        ...
        if content_size > max_file_size:
            return SkillResult(
                success=False,
                error=f"Content too large: {content_size} bytes (limit: {max_file_size} bytes). Split into smaller writes.",
            )
        full_path = self._resolve_path(path, context)
        if not self._is_allowed(full_path, context):
            return SkillResult(success=False, error=f"Access denied: {path}")
...
class FileEditSkill(Skill):
    ...
    parameters = [
        ...
        SkillParameter(name="encoding", type="string", description="エンコーディング", required=False, default="utf-8"),
        SkillParameter(
            name="replace_all", type="boolean", description="全ての出現箇所を置換するか", required=False, default=False
        ),
    ]
    ...
    async def execute(self, context: SkillContext, **kwargs) -> SkillResult:
        ...
        try:
            ...
            if file_size > max_file_size:
                return SkillResult(
                    success=False,
                    error=f"File too large to edit: {file_size} bytes (limit: {max_file_size} bytes). Consider editing a smaller file or splitting the operation.",
                )
    ...
    def _edit_file(self, path: str, old_string: str, new_string: str, encoding: str, replace_all: bool) -> dict:
        ...
        if not replace_all and count > 1:
            return {
                "error": f"old_string found {count} times. Use replace_all=true or provide a more specific string",
                "count": 0,
            }
        if replace_all:
...
class FileListSkill(Skill):
    ...
    parameters = [
        SkillParameter(
            name="path", type="string", description="一覧表示するディレクトリのパス", required=False, default="."
        ),
        SkillParameter(
            name="pattern", type="string", description="フィルタパターン（glob形式）", required=False, default="*"
        ),
        SkillParameter(
            name="recursive", type="boolean", description="再帰的に検索するか", required=False, default=False
        ),
    ...
    async def execute(self, context: SkillContext, **kwargs) -> SkillResult:
        ...
        config_max = context.restrictions.get("max_items", 100)
        max_items = min(max_items, config_max)
        full_path = self._resolve_path(path, context)
        if not self._is_allowed(full_path, context):
            return SkillResult(success=False, error=f"Access denied: {path}")
        try:
