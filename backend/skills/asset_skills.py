...
class ImageGenerateSkill(Skill):
    ...
    parameters = [
        ...
        SkillParameter(
            name="style",
            type="string",
            description="スタイル: pixel_art, anime, realistic, cartoon",
            required=False,
            default="",
        ),
    ]
    ...
    async def execute(self, context: SkillContext, **kwargs) -> SkillResult:
        ...
        result = await asyncio.to_thread(
            self._generate_image, prompt, negative_prompt, width, height, output_path, context
        )
        return result
...
class BgmGenerateSkill(Skill):
    ...
    parameters = [
        ...
        SkillParameter(
            name="style",
            type="string",
            description="スタイル: orchestral, electronic, ambient, rock, jazz",
            required=False,
            default="",
        ),
    ]
    ...
    def _generate_bgm(
        self, prompt: str, duration: float, output_path: Optional[str], context: SkillContext
    ) -> SkillResult:
        try:
            ...
            if result.get("success"):
                ...
                if output_path and result.get("audio_base64"):
                    full_path = (
                        os.path.join(context.working_dir, output_path)
                ...
                return SkillResult(success=True, output=f"BGM生成完了: {prompt[:50]}...", metadata=output_info)
...
class SfxGenerateSkill(Skill):
    ...
    parameters = [
        SkillParameter(
            name="prompt", type="string", description="効果音の説明（例: sword slash, explosion, footsteps）"
        ),
            ...
            if result.get("success"):
                ...
                if output_path and result.get("audio_base64"):
                    ...
                    with open(full_path, "wb") as f:
                        f.write(base64.b64decode(result["audio_base64"]))
                    output_info["saved_to"] = full_path
                return SkillResult(success=True, output=f"効果音生成完了: {prompt[:50]}", metadata=output_info)
...
class VoiceGenerateSkill(Skill):
    ...
    parameters = [
        ...
        SkillParameter(
            name="language", type="string", description="言語: ja, en, zh, ko", required=False, default="ja"
        ),
        SkillParameter(
            name="speaker", type="string", description="話者ID（プロバイダー依存）", required=False, default=""
        ),
        SkillParameter(name="output_path", type="string", description="出力ファイルパス", required=False),
    ...
    def _generate_voice(
        self, text: str, language: str, speaker: str, output_path: Optional[str], speed: float, context: SkillContext
    ) -> SkillResult:
        try:
            ...
            result = provider.synthesize(
                text=text, language=language, speaker_id=speaker if speaker else None, speed=speed
            )
            if result.get("success"):
                ...
                if output_path and result.get("audio_base64"):
                    ...
                    with open(full_path, "wb") as f:
                        f.write(base64.b64decode(result["audio_base64"]))
                    output_info["saved_to"] = full_path
                return SkillResult(success=True, output=f"音声生成完了: {text[:30]}...", metadata=output_info)
