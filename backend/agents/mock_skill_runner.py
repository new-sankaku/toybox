...
class MockSkillRunner(AgentRunner):
    def __init__(self, data_store=None, **kwargs):
        ...
        self._working_dir = kwargs.get("working_dir", None) or os.environ.get(
    ...
    async def run_agent_stream(self, context: AgentContext) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        yield {"type": "progress", "data": {"progress": 5, "current_task": "AI API接続中"}}
            ...
            if tool_calls:
                yield {
                    ...
                    "data": {
                        ...
                        "current_task": f"スキル実行中: {', '.join([tc.name for tc in tool_calls])}",
                    },
                }
                yield {
                    "type": "log",
                ...
                yield {"type": "progress", "data": {"progress": step_progress, "current_task": "AI応答処理中"}}
            await asyncio.sleep(0.3 * self._simulation_speed)
    ...
    async def _execute_mock_asset_skill(
        self, tc: MockToolCall, context: AgentContext, executor: SkillExecutor, handler_config: Dict[str, Any]
    ) -> SkillResult:
