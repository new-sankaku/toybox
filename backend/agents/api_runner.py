...
class ApiAgentRunner(AgentRunner):
    ...
    async def run_agent_stream(self, context: AgentContext) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        yield {"type": "progress", "data": {"progress": 10, "current_task": "プロンプト準備中"}}
        ...
        yield {"type": "progress", "data": {"progress": 20, "current_task": "LLM呼び出し中"}}
        ...
        try:
            ...
            yield {
                "type": "tokens",
                "data": {"count": result.get("tokens_used", 0), "total": result.get("tokens_used", 0)},
            }

            yield {"type": "progress", "data": {"progress": 80, "current_task": "出力処理中"}}
            ...
            yield {"type": "progress", "data": {"progress": 90, "current_task": "承認準備"}}
        ...
        yield {
            "type": "log",
            "data": {"level": "info", "message": "API Agent完了", "timestamp": datetime.now().isoformat()},
        }
    ...
    async def _call_llm(
        self, prompt: str, context: AgentContext, system_prompt: Optional[str] = None
    ) -> Dict[str, Any]:
...
class LeaderWorkerOrchestrator:
    ...
    async def _run_workers_dag(
        ...
        self._emit_progress(
            leader_context.agent_type.value, 30, f"Worker並列実行開始 ({total_workers}タスク, {layer_count}レイヤー)"
        )
    ...
    async def _run_with_quality_check(
        ...
        for retry in range(max_retries):
            ...
            if retry < max_retries - 1:
                ...
                best_idx = max(
                    range(len(result.attempt_history)), key=lambda i: result.attempt_history[i].get("score", 0)
                )
    ...
    async def _perform_quality_check(
        self, output: Dict[str, Any], worker_type: str, project_id: Optional[str] = None
    ) -> QualityCheckResult:
...
統合ドキュメントをMarkdown形式で出力してください。"""
            ...
            if result and result["status"] == "completed":
                ...
                qc_result = await self._perform_quality_check(
                    integrated, leader_context.agent_type.value, leader_context.project_id
                )
