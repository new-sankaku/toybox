...
class PrincipleBasedQualityEvaluator:
    ...
    async def evaluate(self,output:Dict[str,Any],agent_type:str)->Dict[str,Any]:
    ...
    def _rule_based_check(self,content)->Dict[str,Any]:
        issues=[]
        score=1.0

        if not content or len(str(content))<50:
            issues.append("出力内容が不十分です（50文字未満）")
            score-=0.3
    ...
    async def _llm_evaluate(self,output_text:str,rubrics:str)->Dict[str,Any]:
        from providers.registry import get_provider
        from providers.base import AIProviderConfig
        import os

        provider_id=self._settings.get("quality_check_provider","anthropic")
        model=self._settings.get("quality_check_model","claude-haiku-4-5-20250116")
