...
class MockAgentRunner(AgentRunner):
    ...
    async def run_agent_stream(self, context: AgentContext) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        yield {"type": "progress", "data": {"progress": 10, "current_task": "初期化中"}}

        await asyncio.sleep(0.5 * self._simulation_speed)

        yield {"type": "progress", "data": {"progress": 30, "current_task": "処理中"}}
        ...
        yield {"type": "progress", "data": {"progress": 70, "current_task": "出力生成中"}}
        ...
        yield {
            "type": "log",
            "data": {"level": "info", "message": "Mock Agent完了", "timestamp": datetime.now().isoformat()},
        }
    ...
    def _generate_mock_output(self, context: AgentContext) -> Dict[str, Any]:
        ...
        return {
            ...
            "format": "markdown",
            "content": content,
            "metadata": {"mock": True, "agent_type": agent_type, "generated_at": datetime.now().isoformat()},
        }
