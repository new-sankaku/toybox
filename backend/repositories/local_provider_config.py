...
class LocalProviderConfigRepository:
    ...
    def get(self, provider_id: str) -> Optional[LocalProviderConfig]:
        return self.session.query(LocalProviderConfig).filter(LocalProviderConfig.provider_id == provider_id).first()
    ...
    def get_validated_urls(self) -> List[str]:
        configs = self.session.query(LocalProviderConfig).filter(LocalProviderConfig.is_validated == True).all()
