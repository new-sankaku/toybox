...
class LlmJobRepository(BaseRepository[LlmJob]):
    ...
    def claim_job(self, job_id: str) -> Optional[LlmJob]:
        stmt = (
            update(LlmJob)
            .where(and_(LlmJob.id == job_id, LlmJob.status == "pending"))
    ...
    def get_project_token_usage(self, project_id: str) -> int:
        result = (
            self.session.query(func.coalesce(func.sum(LlmJob.tokens_input + LlmJob.tokens_output), 0))
            .filter(and_(LlmJob.project_id == project_id, LlmJob.status == "completed"))
            .scalar()
        )
        return int(result)
    ...
    def cleanup_project_jobs(self, project_id: str) -> int:
        incomplete_jobs = (
            self.session.query(LlmJob)
            .filter(and_(LlmJob.project_id == project_id, LlmJob.status.in_(["pending", "running"])))
            .all()
        )
