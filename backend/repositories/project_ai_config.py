...
class ProjectAiConfigRepository:
    ...
    def get(self, project_id: str, usage_category: str) -> Optional[ProjectAiConfig]:
        return (
            self.session.query(ProjectAiConfig)
            .filter(ProjectAiConfig.project_id == project_id, ProjectAiConfig.usage_category == usage_category)
            .first()
        )

    def get_by_project(self, project_id: str) -> List[ProjectAiConfig]:
        return self.session.query(ProjectAiConfig).filter(ProjectAiConfig.project_id == project_id).all()

    def save(
        self, project_id: str, usage_category: str, provider_id: str, model_id: str, custom_params: dict = None
    ) -> ProjectAiConfig:
    ...
    def delete_all_for_project(self, project_id: str) -> int:
        deleted = self.session.query(ProjectAiConfig).filter(ProjectAiConfig.project_id == project_id).delete()
        self.session.flush()
        return deleted
