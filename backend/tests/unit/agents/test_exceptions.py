...
class TestContextValidationError:
    ...
    def test_with_missing_fields(self):
        error = ContextValidationError("必須フィールドが不足", missing_fields=["project_id", "agent_type"])
...
class TestQualityCheckFailedError:
    def test_recoverable(self):
        error = QualityCheckFailedError("concept_leader", issues=["品質が低い"], retry_count=1)
        assert error.recoverable is True

    def test_not_recoverable(self):
        error = QualityCheckFailedError("concept_leader", issues=["品質が低い"], retry_count=3)
        assert error.recoverable is False
