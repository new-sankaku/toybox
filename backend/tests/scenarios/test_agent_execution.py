...
class TestHealthMonitorIntegration:
    ...
    def test_health_status_tracking(self, mock_sio):
        ...
        monitor._update_health_state(
            "anthropic", HealthCheckResult(available=True, latency_ms=100, checked_at=datetime.now())
        )
        ...
        monitor._update_health_state(
            "anthropic", HealthCheckResult(available=False, error="API Error", checked_at=datetime.now())
        )
