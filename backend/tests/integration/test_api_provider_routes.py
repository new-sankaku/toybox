...
class TestProviderTestConnection:
    def test_test_mock_provider(self, client):
        response = client.post(
            "/api/ai-providers/test", data=json.dumps({"providerType": "mock"}), content_type="application/json"
        )
        assert response.status_code == 200
        data = json.loads(response.data)
    ...
    def test_test_missing_provider_type(self, client):
        response = client.post("/api/ai-providers/test", data=json.dumps({}), content_type="application/json")
