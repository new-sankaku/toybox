...
def normalize_img(img0, img1):
    ...
    # 元の実装は mean/std が fp32 固定で、fp16 の入力を引くと型の格上げで
    # fp32 に戻り、直後の conv が「input Float / weight Half」で落ちる
    mean = torch.tensor([0.485, 0.456, 0.406], dtype=img1.dtype,
                        device=img1.device).view(1, 3, 1, 1)
    std = torch.tensor([0.229, 0.224, 0.225], dtype=img1.dtype,
                       device=img1.device).view(1, 3, 1, 1)
    img0 = (img0 - mean) / std
    img1 = (img1 - mean) / std
